...
function activate(context) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand("multiModalDebug.analyzeActive", async (arg) => {
    ...
    context.subscriptions.push(vscode.commands.registerCommand("multiModalDebug.copyAnalysis", async () => {
        const last = ChatPanel.getLastAnalysis();
        if (!last?.body) {
            vscode.window.showWarningMessage("No analysis available to copy.");
            return;
        }
    ...
    context.subscriptions.push(vscode.commands.registerCommand("multiModalDebug.saveAnalysis", async () => {
        const last = ChatPanel.getLastAnalysis();
        if (!last?.body) {
            vscode.window.showWarningMessage("No analysis available to save.");
            return;
        }
    ...
    // Open side chat
    context.subscriptions.push(vscode.commands.registerCommand("multiModalDebug.openChat", () => {
        ChatPanel.createOrShow(context);
    }));
    // Ping
    context.subscriptions.push(vscode.commands.registerCommand("multiModalDebug.helloWorld", () => {
    ...
    context.subscriptions.push(vscode.commands.registerCommand("multiModalDebug.readFile", async () => {
    ...
    context.subscriptions.push(vscode.commands.registerCommand("multiModalDebug.writeFile", async () => {
...
function getBackendBase() {
    ...
    const deprecated = (cfg.get("backendUrl") || "").trim();
    const base = (deprecated || cfg.get("backendBase") || "http://127.0.0.1:8000").replace(/\/$/, "");
    return base;
}
function getSettings() {
    const cfg = vscode.workspace.getConfiguration("multiModalDebug");
    const maxPayload = cfg.get("maxPayload") ?? 2097152; // ~2MB
    const notebookStrategy = (cfg.get("notebookStrategy") || "cells");
    return { maxPayload, notebookStrategy };
}
/* ---------------- helpers ---------------- */
async function pickFileToRead() {
 ...
 */
function extractNotebookText(path, body) {
    try {
...
function clampText(s, limit = 200000) {
    if (s.length <= limit)
 ...
 */
function prepareTextForAnalysis(path, raw, opts) {
    const { limit = 200000, notebookStrategy = "cells" } = opts || {};
...
async function analyzeWithBackendPayload(payload) {
...
async function analyzeWithBackend(logText) {
    return analyzeWithBackendPayload({ repo: ".", log: logText, screenshot_b64: null });
}
function makeIncidentMarkdown(data, sourcePath) {
...
class ChatPanel {
            ...
            try {
                switch (msg?.type) {
                    ...
                    case "readFile": {
                        const uri = await pickFileToRead();
                        if (!uri)
                            return this.status("read:cancelled");
                    ...
                    case "writeFile": {
                        const uri = await pickFileToWrite();
                        if (!uri)
                            return this.status("write:cancelled");
                        const content = await vscode.window.showInputBox({
                            prompt: "Enter file content",
                            placeHolder: "Type text to save…",
                        });
                    ...
                    case "overwriteFile": {
                        ...
                        const uri = open?.[0];
                        if (!uri)
                            return this.status("overwrite:cancelled");
                        const content = await vscode.window.showInputBox({
                    ...
                    case "analyze": {
                        const sendPath = msg?.sendPath === true;
                        ...
                        if (!hasCached && !sendPath) {
                            ...
                            if (!picked)
                                return this.status("analyze:cancelled");
                        }
                        if (!hasCached && sendPath) {
                        ...
                        await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: "Analyzing via backend…" }, async () => {
                            let result;
                            if (sendPath && path) {
                    ...
                    case "openLocation": {
                        ...
                        const fileField = last?.file;
                        if (!fileField) {
                            this.status("openLocation: no file in last analysis");
                            break;
                        }
                        ...
                        try {
                            ...
                            if (!docUri) {
                                this.status("openLocation: file not found in workspace");
                                break;
                            }
                    ...
                    case "previewReport": {
                        if (!this.lastAnalysis?.body) {
                            this.status("preview: no analysis");
                            break;
                        }
                    ...
                    case "savePatch": {
                        ...
                        if (!patch) {
                            this.status("patch: none available");
                            break;
                        }
                        const ws = vscode.workspace.workspaceFolders?.[0]?.uri;
                        const stamp = new Date().toISOString().replace(/[-:]/g, "").replace(/\..+$/, "");
                        ...
                        if (target) {
                            try {
                                await vscode.workspace.fs.createDirectory(vscode.Uri.joinPath(ws, "patches"));
                            }
                            catch { }
                        }
                        else {
                    ...
                    case "saveTest": {
                        const test = this.lastAnalysis?.body?.test;
                        if (!test) {
                            this.status("test: none available");
                            break;
                        }
                        const ws = vscode.workspace.workspaceFolders?.[0]?.uri;
                        const stamp = new Date().toISOString().replace(/[-:]/g, "").replace(/\..+$/, "");
                        ...
                        if (target) {
                            try {
                                await vscode.workspace.fs.createDirectory(vscode.Uri.joinPath(ws, "tests"));
                            }
                            catch { }
                        }
                        else {
                    ...
                    default:
                        this.status(`unknown:${String(msg?.type)}`);
                }
            }
            catch (err) {
    ...
    static createOrShow(context) {
        ...
        const panel = vscode.window.createWebviewPanel(ChatPanel.viewType, "OurProject-1 Side Chat", column, {
            enableScripts: true,
    ...
    static async postMessage(message) {
        if (!ChatPanel.currentPanel) {
            console.log("[OurProject-1] postMessage dropped (no panel):", message?.type);
            return;
        }
        // keep cache in sync when messages originate from command palette
        if (message.type === "fileContent" || message.type === "fileWritten") {
            ChatPanel.currentPanel.lastPath = message.path;
            ChatPanel.currentPanel.lastBody = message.body ?? "";
        }
        ...
        try {
            ...
            console.log("[OurProject-1] postMessage", message?.type, ok ? "delivered" : "not-delivered");
        }
        catch (err) {
...
ChatPanel.viewType = "multiModalDebug.chat";
function getNonce() {
